...
class Cppo(Algorithm):
    ...
    def _get_policy_for_loss(
        ...
        if continuous:
            # 如果是连续动作空间，获取动作维度并计算logits的形状
            action_dim = self.action_spec[group, "action"].shape[-1]
            logits_shape = list(self.action_spec[group, "action"].shape)
            logits_shape[-1] = action_dim * 2  # 每个动作维度需要输出均值和标准差
        else:
...
@dataclass
class CppoConfig(AlgorithmConfig):
    """CPPO算法的配置类。"""
    ...
    @staticmethod
    def has_centralized_actor() -> bool:
        return True
